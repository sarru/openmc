            ...
            if (check_for_node(node_trigger, "scores")) then
              ...
              if (imomstr > size(MOMENT_STRS)) then
                do imomstr = 1, size(MOMENT_N_STRS)
                  if (starts_with(score_name,trim(MOMENT_N_STRS(imomstr)))) then
                    ...
                    score_name(n_order_pos:(len_trim(score_name)))),4)
                    if (n_order > MAX_ANG_ORDER) then
                      ! User requested too many orders; throw a warning and set
                      ! to the maximum order.
                      ! The above scheme will essentially take the absolute
                      ! value
                      call warning("Invalid scattering order of " &
                           &// trim(to_str(n_order)) // " requested. Setting to&
                           & the maximum permissible value, " &
              ...
              case ('all')
                if (t % n_user_triggers /= 1) then
                  call fatal_error("Cannot set trigger for all and other&
                       & scoring functions in the same tally. Separate scoring&
                       & functions into distinct tallies")
                else
                  t % trigger_for_all = .true.
                end if
              case ('flux')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_FLUX)
              case ('flux-yn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_FLUX_YN)
              case ('total')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_TOTAL)
              case ('total-yn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_TOTAL_YN)
              case ('scatter')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_SCATTER)
              case ('nu-scatter')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_NU_SCATTER)
              case ('scatter-n')
                if (n_order == 0) then
                  t % score(trig_ind) % position = &
                       &find_score % get_key(SCORE_SCATTER)
                  ...
                  t % score(trig_ind) % position = &
                       &find_score % get_key(SCORE_SCATTER_N)
                end if
              case ('nu-scatter-n')
                if (n_order == 0) then
                  t % score(trig_ind) % position = &
                  ...
                  t % score(trig_ind) % position = &
                       &find_score % get_key(SCORE_NU_SCATTER_N)
                end if
              case ('scatter-pn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_SCATTER_PN)
              case ('nu-scatter-pn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_NU_SCATTER_PN)
              case ('scatter-yn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_SCATTER_YN)
              case ('nu-scatter-yn')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_NU_SCATTER_YN)
              case('transport')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_TRANSPORT)
              case ('n1n')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_N_1N)
              case ('n2n')
                t % score(trig_ind) % position = &
                     &find_score % get_key(N_2N)
              case ('n3n')
                t % score(trig_ind) % position = &
                     &find_score % get_key(N_3N)
              case ('n4n')
                t % score(trig_ind) % position = &
                     &find_score % get_key(N_4N)
              case ('absorption')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_ABSORPTION)
              case ('fission')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_FISSION)
              case ('nu-fission')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_NU_FISSION)
              case ('kappa-fission')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_KAPPA_FISSION)
              case ('current')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_CURRENT)
              case ('events')
                t % score(trig_ind) % position = &
                     &find_score % get_key(SCORE_EVENTS)
              end select


              if (.not. t % trigger_for_all &
                   .and. t % score(trig_ind) % position == 0) then
                call fatal_error("The score " // trim(score_name) // " has not &
                     &been set in tally " // trim(to_str(t % id)) &
                     &// " in tally XML file.")
              end if
